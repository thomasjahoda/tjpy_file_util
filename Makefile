...
flake8: ## check style with flake8 (lint)
	flake8 tjpy_file_util tests --max-line-length=120

mypy: ## check types with mypy
	mypy tjpy_file_util tests
...
coverage: ## check code coverage with the current python env
    pytest --cov=tjpy_file_util --cov-report=xml:.dev/coverage/coverage.xml
	coverage report -m
	coverage html --directory .dev/coverage/htmlcov
	$(BROWSER) .dev/coverage/htmlcov/index.html

docs: ## generate Sphinx HTML documentation, including API docs
	rm -f docs/tjpy_file_util.rst
	rm -f docs/modules.rst
	sphinx-apidoc -o docs/ tjpy_file_util
	$(MAKE) -C docs clean
	$(MAKE) -C docs html
